...
def parse_cmdline(argv=None):
    ...
    parser.add_argument(
        "module_name",
        help=("The name of module/repository where" " the file to be checked lives."),
    )
    parser.add_argument(
        "filenames",
        metavar="filename",
    ...
    parser.add_argument(
        ...
        help=("The system config file to use" " (default: %(default)s)"),
    )
    parser.add_argument(
        "--config",
        "-c",
        ...
        metavar="CONFIG_FILENAME",
        help=(
            "An additional configuration file providing" " module-specific settings."
        ),
    )
    parser.add_argument(
        "--max-files-with-errors",
        metavar="N",
        dest="max_files_with_errors",
        type=int,
        default=MAX_FILES_WITH_STYLE_FAILURES,
        help=(
            "Indicates the number of files with style"
    ...
    debug.add_argument(
        ...
        type=int,
        help=(
            "Option to pretend today is a different year"
    ...
    debug.add_argument(
        "--dump-checks",
        nargs=0,
        action=DumpChecksAction,
        help=("Dump the list of checks performed for each" " kind of file."),
    )
    ...
    m = re.search(
        r"(/cvs/Dev|^trunk|^branches(/global/[^/]*)?)/([^/]*)", args.module_name
    )
    if m is not None:

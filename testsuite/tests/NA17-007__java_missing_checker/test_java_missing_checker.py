...
def test_no_checker(style_checker):
    ...
    try:
        ...
        p = style_checker.run_style_checker('notimportant', 'empty.java',
                                   use_sys_executable=True)
        style_checker.assertNotEqual(p.status, 0, p.image)
        style_checker.assertRunOutputEqual(p, """\
Failed to run checkstyle: [Errno 2] No such file or directory: 'checkstyle'
""")
    finally:

...
def test_no_pyflakes(style_checker):
    ...
    # where pyflakes is missing, except the testsuite uses a version
    # of Python which we expect to provide pyflakes.
    #
    ...
    # calls pyflakes via self.executable.
    #
    # So, instead, what we do create a temporary copy of the Python
    # install in our testsuite directory where the pyflakes module
    ...
    style_checker.make_minimal_copy_of_python_install(
        target_dir=local_python_dir, exclude_modules=("pyflakes",),
    )
    ...
    try:
        ...
        style_checker.assertRunOutputEqual(
            p,
            f"""\
{local_python_exe}: No module named pyflakes
""",
        )
    finally:

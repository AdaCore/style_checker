...
/* These two variables are used to defer recursively expanding incomplete types while we are processing a record or subprogram type.  */
...
static void copy_alias_set		PARAMS ((tree, tree));
  ...
  switch (kind)
	...
	/* If this is a public constant or we're not optimizing and we're not
	   making a VAR_DECL for it, make one just for export or debugger
           ...
           aliased.  If only the address is taken, only do this if we are
           defining the entity since Address_Taken will differ between
           package and client.  */
        if (TREE_CODE (gnu_decl) == CONST_DECL
          ...
          DECL_CONST_CORRESPONDING_VAR (gnu_decl)
            = create_var_decl (gnu_entity_id, gnu_ext_name, gnu_type,
                               gnu_expr, 0, Is_Public (gnat_entity),
                               ! definition, static_p, 0);
	...
	if (gnu_type == 0 && gnu_desig_type != 0)
	  {
	    ...
	    gnu_type
	      = build_pointer_type_for_mode (gnu_desig_type, ptr_mode,
					     No_Strict_Aliasing (gnat_entity));
	  }
...
static void
copy_alias_set (gnu_new_type, gnu_old_type)
     tree gnu_new_type;
     tree gnu_old_type;
{
     ...
     Node_Id gnat_node;
{
  ...
  if (TREE_CODE (gnu_operand) == UNCONSTRAINED_ARRAY_REF)
	...
	= TREE_READONLY (TREE_TYPE (TREE_TYPE (gnu_operand)));
      return gnu_result;
    }
  else
